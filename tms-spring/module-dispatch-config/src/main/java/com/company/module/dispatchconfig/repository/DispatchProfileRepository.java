...
public interface DispatchProfileRepository extends JpaRepository<DispatchProfile, Long> {
    ...
    @Query(value = "SELECT COALESCE(MAX(SORT_SEQ),0)+1 FROM KNRAWMS.TMS_DS_DISPATCH_PROFILE", nativeQuery = true)
    int nextSortSeq();

    @Modifying
    @Query(value = "UPDATE KNRAWMS.TMS_DS_DISPATCH_PROFILE SET SORT_SEQ=:seq WHERE PROFILE_ID=:id", nativeQuery = true)
